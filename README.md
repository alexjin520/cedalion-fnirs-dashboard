...
```text
连续信号：SNIRF → 有效光强通道 → OD → HbO/HbR → 可选 TDDR / CBSI / Wavelet 比较 → 0.01–0.5 Hz 滤波
质量评估：原始光强 → SNR / SCI / PSP + GVTD → 自动与人工通道筛选、运动候选区段
任务平均：事件区间 → OD → TDDR → HbO/HbR → 可选 CBSI → 滤波 → 分段 → 基线校正 → 条件响应
任务 GLM：事件区间 → TDDR → HbO/HbR → 可选 CBSI → Gamma HRF + 余弦漂移 (+ 可选短间距、辅助生理或全局回归) → beta / 95% CI / t / p / FDR q
```
...
`FNIRS_DATA_DIR` 下的所有 `.snirf` 文件会出现在总览页“分析记录”选择器中。选择后
...
```bash
.conda-env/bin/python server.py --host 0.0.0.0 --port 8080
```
...
浏览器访问 `http://服务器IP:8080`，健康检查为 `/api/health`；健康检查只确认所选（或默认）文件存在，不触发完整分析。
...
- `/api/task-glm`：指定条件、通道和条件对比的 GLM 统计量
- `/api/task-glm-export`：所有 GLM 条件效应与条件对比的 CSV
...
进入任务分析。只有一个区间的条件只展示单次响应，不提供 SEM，也不作为重复
试次统计结果。

设备记录可能包含完整的光源×探测器矩阵，并以零填充未使用组合。服务端只纳入
每个波长至少 99% 采样为正值的通道，再对入选通道内的孤立非正值作线性插值，
避免对非正光强取对数。总览和“质量与记录”页会同时显示原始与实际分析通道数。
...
- GLM：默认 OLS、Gamma HRF `sigma=3 s`、余弦漂移截止 `0.003 Hz`、短间距回归模式 `auto`
...
JSON，记录输入文件、软件版本、完整参数、光强和质量筛选排除、探头/短间距状态、
